        ...
        if not hasChannels and hasLibrary:
            ...
            while not MONITOR().abortRequested():
                ...
                if   retval == 0: return True #No
                    ...
                    # madteevee (imports fork): replaced eval(menu[select].getPath())
                    # with an explicit dispatch table. The original pattern
                    # treated a list-item url as Python source. Safe today
                    # because the four urls below are hard-coded, but any
                    # future menu item whose url came from external data
                    # (channel name, import title, server discovery) would
                    # turn this into arbitrary code execution.
                    _dispatch = {'__manager':  __manager,
                    ...
                    if not select is None:
                        try:
                            _key = menu[select].getPath()
                            _fn  = _dispatch.get(_key)
                            if _fn is not None: return _fn()
                            log("Create: _auto, unknown menu path %s"%(_key), xbmc.LOGERROR)
