...
from lang_codes import LANG_CHOICES, normalize_lang
#todo pinlock
...
# madteevee fork: ForceSubtitles extended with Smart-by-Audio-Language mode.
# Legacy option 0 (Force Subtitles bool) preserved for back-compat — only
# read when Mode (option 1) is "Use Legacy Toggle". Six new options drive
# per-item plan generation at build time (BUILD_FILELIST_POST action); the
# plan is consumed at playback by Player.setSubtitles / _applySmartSubsFromPlan.
# See lang_codes.py for ISO 639-1/2 normalization.
SMART_MODES = {
    'Use Legacy Toggle':         'legacy',
    'Always On':                 'always_on',
    'Always Off':                'always_off',
    'Smart by Audio Language':   'smart',
}

SMART_FALLBACKS = {
    'First available':           'first',
    'Off':                       'off',
    'Kodi default':              'default',
}

SMART_UNKNOWN_AUDIO = {
    'Treat as foreign (subs on)':   'foreign',
    'Treat as native (subs off)':   'native',
    'Use legacy toggle value':      'legacy',
}
...
class ForceSubtitles(BaseRule):
    def __init__(self):
        ...
        self.description        = "Subtitle and audio policy (legacy toggle or Smart by Audio Language)."
        self.optionLabels       = [
            'Force Subtitles (legacy)',
            'Mode',
            'Native audio language',
            'Preferred subtitle language',
            'Preferred audio language',
            'Fallback when no matching sub',
            'Unknown audio language policy',
        ]
        self.optionValues       = [
            BUILTIN.isSubtitle(),
            'legacy',
        ...
        self.optionDescriptions = [
            'Honored only when Mode = Use Legacy Toggle. Identical to the original Force Subtitles behavior.',
            'Use Legacy Toggle reproduces the original rule. Smart by Audio Language reads the file metadata at build time and decides per program.',
            'Audio in this language is treated as understood: subs OFF. (Smart mode only.)',
            'Pick the sub stream matching this language when subs are turned on. (Smart mode only.)',
            'If the file has audio in this language, switch to it before evaluating subs. (Smart mode only.)',
            'What to do in Smart mode when no subtitle stream matches the preferred sub language.',
            'What to do in Smart mode when the audio stream has no language tag (empty / und).',
        ]
        self.actions            = [
    ...
    def getTitle(self):
        mode = self._modeValue()
        if mode == 'legacy':
            return '%s (%s)'%(self.name,{True:LANGUAGE(30184),False:LANGUAGE(30021)}[bool(self.optionValues[0])])
        return '%s (Mode: %s)'%(self.name, mode)
    ...
    def _modeValue(self):
        v = (self.optionValues[1] if len(self.optionValues) > 1 else 'legacy')
        return v if v in ('legacy','always_on','always_off','smart') else 'legacy'


    def _planForItem(self, item):
        """
        Compute the smart_subs plan for one fileList entry. Returns a dict or None
        when no plan is needed (item has no streamdetails → caller leaves the item
        alone; playback falls through to legacy behavior).
        """
        streamdetails = (item.get('streamdetails') or {})
        audio_streams = streamdetails.get('audio') or []
        sub_streams   = streamdetails.get('subtitle') or []
        if not audio_streams: return None

        # Normalize all detected languages to ISO 639-1
        audio_langs = []
        for s in audio_streams:
            l = normalize_lang(s.get('language',''))
            if l: audio_langs.append(l)
        sub_langs = []
        for s in sub_streams:
            l = normalize_lang(s.get('language',''))
            if l: sub_langs.append(l)

        native      = normalize_lang(self.optionValues[2])
        pref_sub    = normalize_lang(self.optionValues[3])
        pref_audio  = normalize_lang(self.optionValues[4])
        fallback    = self.optionValues[5] if self.optionValues[5] in ('first','off','default') else 'first'
        unknown_pol = self.optionValues[6] if self.optionValues[6] in ('foreign','native','legacy') else 'foreign'
        legacy_bool = bool(self.optionValues[0])

        # Effective audio language: if we want to switch to a preferred audio and
        # that language exists on the file, evaluate AGAINST the preferred. Otherwise
        # the first detected language is the de-facto default Kodi will pick.
        will_switch_audio = bool(pref_audio and pref_audio in audio_langs)
        effective_audio   = pref_audio if will_switch_audio else (audio_langs[0] if audio_langs else '')

        # Subs on/off
        if not effective_audio:
            if   unknown_pol == 'foreign': sub_state = True;        rationale = 'unknown_audio_foreign'
            elif unknown_pol == 'native':  sub_state = False;       rationale = 'unknown_audio_native'
            else:                          sub_state = legacy_bool; rationale = 'unknown_audio_legacy'
        else:
            is_native = (native and effective_audio == native)
            sub_state = not is_native
            rationale = 'native_audio' if is_native else 'foreign_audio'

        # Pick sub language target + apply fallback if needed
        sub_lang_target = ''
        if sub_state:
            if pref_sub and pref_sub in sub_langs:
                sub_lang_target = pref_sub
                rationale += '+pref_sub_match'
            elif sub_streams:
                if   fallback == 'first':
                    if sub_langs:
                        sub_lang_target = sub_langs[0]
                        rationale += '+fallback_first'
                    else:
                        rationale += '+fallback_first_unknown_lang'
                elif fallback == 'off':
                    sub_state = False
                    rationale += '+fallback_off'
                else:  # 'default'
                    rationale += '+fallback_kodi_default'
            else:
                # No sub streams scraped — Kodi may still auto-load external subs at playback
                rationale += '+no_sub_streams_in_metadata'

        return {
            'sub_state':         sub_state,
            'sub_lang_target':   sub_lang_target,
            'audio_lang_target': pref_audio if will_switch_audio else '',
            'rationale':         rationale,
        }


    def _formatLabel(self, plan):
        if not plan: return ''
        bits = []
        a = (plan.get('audio_lang_target') or '').upper()
        if a: bits.append('Audio %s'%(a))
        if plan.get('sub_state'):
            s = (plan.get('sub_lang_target') or '').upper()
            bits.append('Subs %s'%(s) if s else 'Subs ON')
        else:
    ...
    def runAction(self, actionid, citem, parameter, inherited):
        if actionid == RULES_ACTION_PLAYER_START:
            # parameter is the playingItem dict.
            mode = self._modeValue()
            self.storedValues[0] = inherited.lastSubState
            inherited.smartSubsMode = mode
            if mode == 'legacy':
                inherited.lastSubState = bool(self.optionValues[0])
                self.log("runAction PLAYER_START, mode=legacy lastSubState=%s"%(inherited.lastSubState))
            else:
                self.log("runAction PLAYER_START, mode=%s"%(mode))

        elif actionid == RULES_ACTION_PLAYER_STOP:
            inherited.lastSubState  = self.storedValues[0]
            inherited.smartSubsMode = 'legacy'
            self.log("runAction PLAYER_STOP, restored lastSubState=%s, smartSubsMode=legacy"%(inherited.lastSubState))

        elif actionid == RULES_ACTION_CHANNEL_BUILD_FILELIST_POST:
            # parameter is the fileList (list of fitems). For Smart mode write a
            # smart_subs plan + smart_subs_label to each item with usable
            # streamdetails. For any other mode, scrub stale plans left over
            # from a previous Smart build.
            mode = self._modeValue()
            if isinstance(parameter, list):
                if mode == 'smart':
                    n_planned = 0
                    for it in parameter:
                        plan = self._planForItem(it)
                        if plan:
                            it['smart_subs']       = plan
                            it['smart_subs_label'] = self._formatLabel(plan)
                            n_planned += 1
                        else:
                            it.pop('smart_subs', None)
                            it.pop('smart_subs_label', None)
                    self.log("[%s] BUILD_FILELIST_POST: planned %d/%d items"%(citem.get('id'), n_planned, len(parameter)))
                else:
                    for it in parameter:
                        it.pop('smart_subs', None)
                        it.pop('smart_subs_label', None)
