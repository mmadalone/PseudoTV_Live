...
class Resources(object):
    ...
    def __del__(self):
        # madteevee (imports fork): hard-timeout watchdog. The plain
        # SETTINGS.setCacheSetting('imageCache', ...) call wedged the
        # chkLibrary executor thread on 2026-05-11 07:23:13 — cacheDB.set
        # blocked inside SQLite for 7+ minutes, freezing GC on whichever
        # thread happened to drop the last reference to a Resources object.
        # Symptom: library walk toast stuck at 10% (TV Networks iteration),
        # Builder couldn't pick up SleepyTV's stale changed=True flag because
        # chkLibrary → chkChannels chain was blocked behind the wedged write.
        # Snapshot the dict on the calling thread (cheap), run the actual
        # write on a daemon thread with a 2s join budget; if the cache is
        # wedged, abandon the write rather than freeze the caller. Losing
        # one imageCache write is harmless — it's re-seeded from disk on
        # the next Resources() construction.
        try:
            cache_snapshot = dict(self.imageCache)
        except Exception:
            return
        def _persist():
            try: SETTINGS.setCacheSetting('imageCache', cache_snapshot)
            except Exception: pass
        try:
            t = Thread(target=_persist, name='Resources.__del__.imageCache')
            t.daemon = True
            t.start()
            t.join(timeout=2.0)
            if t.is_alive():
                try: self.log('__del__, imageCache persist timed out (>2s) — abandoning write to unblock GC', xbmc.LOGWARNING)
                except Exception: pass
            else:
                try: self.log(f'__del__, imageCache = {len(cache_snapshot)}')
                except Exception: pass
        except Exception: pass
    ...
    def getNames(self, chname: str, type: str="Custom") -> list:
        ...
        # imports fork (madteevee): also generate underscore + dash forms of every
        # variant so resource-image addons that store filenames with separator
        # characters (e.g. `Cartoon_Cartoons.png` in the madteevee logos resource)
        # are auto-resolved. Wikipedia/file-naming-style separators are common.
        for v in list(final_results):
            if ' ' in v:
                final_results.add(v.replace(' ', '_'))
                final_results.add(v.replace(' ', '-'))
