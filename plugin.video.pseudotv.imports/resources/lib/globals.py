...
# madteevee (imports fork): unified ownership predicate for cleanSelf. m3u.py
# and xmltvs.py used to keep separate definitions of "which IDs are ours" and
# they had drifted — m3u accepted any-length suffix matches, xmltvs hard-coded
# 32-char (channels) and 16-char (recordings) prefix lengths. A theoretical
# 16-char `@PseudoTV_Live` id would be kept by m3u and dropped by xmltvs ->
# channel exists in M3U but EPG is blank. Centralising the suffix check here
# guarantees the two writers agree on ownership; format-specific length checks
# stay in the callers where the format distinction actually lives.
def isOwnedID(item_id, slug='@%s'%(PSEUDOTV_SLUG), import_slugs=None):
    if not item_id: return False
    if slug and item_id.endswith(slug): return True
    if not import_slugs: return False
    return any(item_id.endswith(suf) for suf in import_slugs)
...
def isCenterlized():
    default = 'special://profile/addon_data/%s/cache'%(ADDON_ID)
