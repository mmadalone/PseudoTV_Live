...
class JSONRPC(object):
    ...
    def getInfoLabels(self, keys, cache=False):
        # Consolidation perf (madteevee): batched form of getInfoLabel.
        # XBMC.GetInfoLabels accepts a list of labels in one call. Used by the
        # System Info endpoint to collapse 2 sequential roundtrips into 1.
        if not keys: return {}
        param = {"method":"XBMC.GetInfoLabels","params":{"labels":list(keys)}}
        if cache: return self.cacheJSON(param).get('result',{}) or {}
        else:     return self.sendJSON(param).get('result',{}) or {}
    ...
    def getInfoBooleans(self, keys, cache=False):
        # Step 6 Phase 2 perf (madteevee): batched form of getInfoBool. Kodi's
        # XBMC.GetInfoBooleans accepts a list of keys in one call. We were
        # spending ~10s on cold settings-schema fetches doing 10 single-key
        # calls; batching collapses that to a single roundtrip.
        if not keys: return {}
        param = {"method":"XBMC.GetInfoBooleans","params":{"booleans":list(keys)}}
        if cache: return self.cacheJSON(param).get('result',{}) or {}
        else:     return self.sendJSON(param).get('result',{}) or {}
