...
#info
ADDON_ID            = 'plugin.video.pseudotv.imports'
...
CHANNEL_LIMIT       = 9999  # imports fork (madteevee): bumped 999→9999 per plan H1.
                            # Allows disabled-channel bucket at 9000+ (operator
                            # request) and gives 10x headroom for multi-import
                            # setups. Audited: zero functional impact —
                            # manager.py:159's `range(CHANNEL_LIMIT)` allocates
                            # ~80KB of ints (instant on RPi5); manager.py:309/965
                            # gates still fire correctly with new bound.
...
SETTINGS_XML_PATH   = os.path.join(ADDON_PATH,'resources','settings.xml')           # imports fork (madteevee): schema source for settings_schema.py
STRINGS_PO_PATH     = os.path.join(ADDON_PATH,'resources','language','resource.language.en_gb','strings.po')
