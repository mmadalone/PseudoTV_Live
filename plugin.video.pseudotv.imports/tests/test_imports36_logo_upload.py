...
def test_manager_html_has_upload_button_in_cf_modal():
    """imports.36: Custom-edit modal cf-logo input has 📤 upload button."""
    src = _read(HTML)
    m = re.search(
        r'<input type="text" id="cf-logo".*?logo-upload-input',
        src, re.DOTALL,
    )
    assert m, (
        "imports.36: Custom-edit modal must have an upload-logo button + "
        "hidden file input alongside the cf-logo input"
    )
