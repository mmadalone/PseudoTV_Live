...
def addon_settings(addon_id=None):
    ...
    # Credentials handling — tolerant of missing settings[ADDON_ID] for
    # smoke-test setups (PseudoTV doesn't use this path; only kept for
    # vrt.nu-pattern compatibility).
    if 'ADDON_USERNAME' in os.environ and 'ADDON_PASSWORD' in os.environ:
        settings.setdefault(ADDON_ID, {})['username'] = os.environ.get('ADDON_USERNAME')
        settings.setdefault(ADDON_ID, {})['password'] = os.environ.get('ADDON_PASSWORD')
        ...
        settings.setdefault(ADDON_ID, {}).update(credentials)
    # Note: vrt.nu prints an error when credentials.json is absent; we silently
    # skip because PseudoTV has no auth flow that needs credentials.

    if addon_id:
        return settings.get(addon_id, {})
...
def import_language(language):
    ...
    try:
        podb = polib.pofile('plugin.video.pseudotv.live/resources/language/{language}/strings.po'.format(language=language))
    except IOError:
        podb = polib.pofile('plugin.video.pseudotv.live/resources/language/resource.language.en_gb/strings.po')
...
ADDON_INFO = read_addon_xml('plugin.video.pseudotv.live/addon.xml')
