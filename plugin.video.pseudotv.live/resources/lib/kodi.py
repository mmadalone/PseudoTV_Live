...
class Settings:
    ...
    def openGuide(self, instance=ADDON_NAME):
        def __match(label):
            items = jsonRPC.getDirectory({"directory":baseURL}).get('files',[])  # noqa: F821 — closure over enclosing-scope jsonRPC bound below
            for item in items:
...
class Builtin:
    ...
    def executescript(self, path):
        ...
        # CFileUtils::Exists() fails — see ExecuteAsync log warning. RunScript()
        # builtin parses commas and feeds tokens as sys.argv to the script.
